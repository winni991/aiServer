...
func (h MysqlInitHandler) WriteConfig(ctx context.Context) error {
	...
	global.AI_CONFIG.Mysql = c
	global.AI_CONFIG.JWT.SigningKey = uuid.Must(uuid.NewV4()).String()
	cs := utils.StructToMap(global.AI_CONFIG)
	for k, v := range cs {
		global.AI_VP.Set(k, v)
	}
	return global.AI_VP.WriteConfig()
}
